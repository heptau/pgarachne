...
- `GET /{prefix}/:database/openapi.yaml` (or `?format=yaml` on either route) returns the same OpenAPI document as YAML, via `github.com/goccy/go-yaml`.

### Changed

- **Breaking:** `GET /{prefix}/:database/openapi.json` now requires the same authentication as `/jsonrpc` (Basic / Bearer JWT / API token) and the returned spec is filtered to only the methods the authenticated role may execute — mirroring how MCP `tools/list` already behaves per role. Previously this endpoint was unauthenticated and always listed every method regardless of caller. This required removing `SECURITY DEFINER` from `pgarachne.generate_openapi_spec()` (it is now `SECURITY INVOKER`, the default): a `SECURITY DEFINER` function forces `current_user` to its owner for its entire execution, including nested calls, which was silently defeating `capabilities()`'s own `has_function_privilege(current_user, ...)` filter. Also fixed `servers[0].url` in the generated spec, which previously concatenated the base URL with the full `/jsonrpc` path even though every `paths` key is already a full absolute path — doubling up the effective URL per OpenAPI 3.1's `server.url + path key` resolution rule; it is now the bare origin.
...
- **Breaking:** `go.mod`: the `go` directive moved from `1.25.0` to `1.26.0`. This wasn't a deliberate policy bump — routine dependency updates pulled in `github.com/quic-go/quic-go` v0.62.0 (an indirect dependency, via `gin-gonic/gin`'s optional HTTP/3 support), which itself requires `go1.26.0`, and `go mod tidy` raised the module's own floor to match. Building this module now requires a `go1.26.0`+ toolchain.
- `go.mod`/`go.sum`: routine dependency refresh. `github.com/prometheus/client_golang` v1.23.2 → v1.24.1 (and its transitive `prometheus/common` v0.66.1 → v0.71.0, `prometheus/procfs` v0.16.1 → v0.22.0), `quic-go` v0.60.0 → v0.62.0, `go.mongodb.org/mongo-driver/v2` v2.7.0 → v2.8.2, `golang.org/x/{crypto,net,sys,text,arch}` and the remaining transitive deps to their latest releases. The direct dependencies `gin-gonic/gin`, `gin-contrib/cors`, `golang-jwt/jwt/v4`, `joho/godotenv`, and `lib/pq` were already at their latest stable release — no change. `govulncheck` reports one pre-existing advisory (`GO-2026-5932`, the unmaintained `golang.org/x/crypto/openpgp` package) with no fix available; this repo doesn't import that package.
- CI (`.github/workflows/ci.yml`): `setup-go`'s floating minor bumped `'1.25'` → `'1.26'` to track the new `go.mod` floor; the pinned `golangci-lint` version bumped `v2.12.2` → `v2.13.2` (needed to build against a `go1.26.0` target); `codecov/codecov-action` bumped `@v5` → `@v7` (two majors behind — `@v5`/`@v6` were exact pins, not floating, so they never moved on their own; no config changes were required for this repo's usage).
...
- Docs (`real-time-notifications.html`, `security-roles.html`) and `AGENTS.md`: documented that the SSE endpoint, unlike JSON-RPC and MCP, does not apply `SET LOCAL ROLE` or any per-channel authorization — all SSE clients for a database share one `LISTEN` connection opened as `DB_USER`, so any authenticated caller can subscribe to any `NOTIFY` channel and see everything published on it, regardless of their role's actual grants. This was previously undocumented behaviour that contradicted the "PgArachne relies entirely on the PostgreSQL permission system" claim on the security page for this one endpoint; no code changed — `LISTEN`/`NOTIFY` channels aren't database objects with their own `GRANT`s to enforce, so operators who need channel-level access control must implement it themselves (e.g. per-role channel naming, or keeping sensitive data out of `NOTIFY` payloads).

### Fixed

- `internal/database/database.go`: direct-credential (`Authorization: Basic`) connection pools were never removed from the `directPools` cache except at process shutdown or when `DIRECT_POOL_LIMIT` was reached, even though each pool's own connections idle out within `ConnMaxIdleTime` (2 min). On a long-running instance, routine password rotation across users eventually filled the cache with long-dead entries and started rejecting *every* new direct-auth credential with "direct connection pool limit reached", regardless of how idle the cached pools actually were. `GetUserConnection` now evicts the least-recently-used pool with zero open connections to make room before failing closed; a pool currently serving a request (`OpenConnections > 0`) is never evicted out from under it.
...
- `sql/universal_table_access.sql`: `api.universal_create` was broken on PostgreSQL 16 — `json_populate_record` has no `jsonb` variant, `RETURNING row_to_json(*)` is invalid, and `INSERT ... SELECT *` overwrote serial-PK columns with NULL (column defaults never applied). The function now derives the column list from the JSON keys and the column types from the catalog, so columns omitted from the payload (serial PK, `created_at`, …) receive their `DEFAULT` values.
